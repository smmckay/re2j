...
/**
 * A stateful iterator that interprets a regex {@code Pattern} on a specific input. Its interface mimics the JDK 1.4.2
 * {@code java.util.regex.Matcher}.
 *
 * <p>
 * Conceptually, a Matcher consists of four parts:
 * <ol>
 * <li>A compiled regular expression {@code Pattern}, set at construction and fixed for the lifetime of the
 * matcher.</li>
 *
 * <li>The remainder of the input string, set at construction or {@link #reset()} and advanced by each match operation
 * such as {@link #find}, {@link #matches} or {@link #lookingAt}.</li>
 *
 * <li>The current match information, accessible via {@link #start}, {@link #end}, and {@link #group}, and updated by
 * each match operation.</li>
 *
 * <li>The append position, used and advanced by {@link #appendReplacement} and {@link #appendTail} if performing a
 * search and replace from the input to an external {@code StringBuffer}.
 *
 * </ol>
 *
 * <p>
 * See the <a href="package.html">package-level documentation</a> for an overview of how to use this API.
 * </p>
 *
...
public final class Matcher {
     ...
     *
     * @param group
     *            the group index; 0 is the overall match
     * @throws IllegalStateException
     *             if there is no match
    ...
    private void loadGroup(int group) {
        ...
        boolean ok = pattern.re2().match(inputSequence, groups[0], end, anchorFlag, groups, 1 + groupCount);
    ...
    /**
     * Matches the entire input against the pattern (anchored start and end). If there is a match, {@code matches} sets
     * the match state to describe it.
     *
    ...
    /**
     * Matches the beginning of input against the pattern (anchored start). If there is a match, {@code lookingAt} sets
     * the match state to describe it.
     *
    ...
    /**
     * Matches the input against the pattern (unanchored). The search begins at the end of the last match, or else the
     * beginning of the input. If there is a match, {@code find} sets the match state to describe it.
     *
     ...
     * {@code find} sets the match state to describe it.
     *
     * @param start
    ...
    /**
     * Quotes '\' and '$' in {@code s}, so that the returned string could be used in {@link #appendReplacement} as a
     * literal replacement of {@code s}.
     *
     * @param s
    ...
    /**
     * Appends to {@code sb} two strings: the text from the append position up to the beginning of the most recent
     * match, and then the replacement with submatch groups substituted for references of the form {@code $n}, where
     * {@code n} is the group number in decimal. It advances the append position to where the most recent match ended.
     *
     * <p>
     * To embed a literal {@code $}, use \$ (actually {@code "\\$"} with string escapes). The escape is only necessary
     * when {@code $} is followed by a digit, but it is always allowed. Only {@code $} and {@code \} need escaping, but
     * any character can be escaped.
     *
     * <p>
     * The group number {@code n} in {@code $n} is always at least one digit and expands to use more digits as long as
     * the resulting number is a valid group number for this pattern. To cut it off earlier, escape the first digit that
     * should not be used.
     *
     * @param sb
     ...
     * @return the {@code Matcher} itself, for chained method calls
     * @throws IllegalStateException
     *             if there was no most recent match
     * @throws IndexOutOfBoundsException
    ...
    /**
     * Appends to {@code sb} the substring of the input from the append position to the end of the input.
     *
     * @param sb
     *            the {@link StringBuffer} to append to
    ...
    /**
     * Returns the input with all matches replaced by {@code replacement}, interpreted as for {@code appendReplacement}.
     *
     * @param replacement
     *            the replacement string
